...
const THEMES = ['light', 'dark', 'system', 'beranda']
let layoutNavbarVar
...
class TemplateCustomizer {
  ...
  _setup(_container = document) {
    ...
    if (!this._hasControls('theme')) {
      ...
      // themeCb
      const themeCb = e => {
        if (e.target.value === 'beranda') {
          // Beranda theme: dark mode + premium skin
          document.documentElement.setAttribute('data-bs-theme', 'dark')
          document.documentElement.setAttribute('data-skin', 'premium')
          this.setTheme('dark', true)
          this.setSkin('premium', true)

          // Sync skin radio button
          const skinInput = this.container.querySelector('.template-customizer-skins-options input[value="premium"]')
          if (skinInput) {
            skinInput.checked = true
            if (window.Helpers && typeof window.Helpers.updateCustomOptionCheck === 'function') {
          ...
          if (semiDarkL) semiDarkL.classList.add('d-none')

          // Sync toggles
          if (window.Helpers && typeof window.Helpers.syncThemeToggles === 'function') {
            window.Helpers.syncThemeToggles('dark')
          }
        } else {
    ...
    if (!this._hasControls('skins')) {
      ...
      const skinCb = e => {
        ...
        // Auto-switch to dark theme when premium skin is selected
        if (e.target.value === 'premium') {
          document.documentElement.setAttribute('data-bs-theme', 'dark')
          this.setTheme('dark', true)

          // Sync the theme radio button in customizer UI
          const themeInput = this.container.querySelector('.template-customizer-themes-options input[value="dark"]')
          if (themeInput) {
            themeInput.checked = true
            if (window.Helpers && typeof window.Helpers.updateCustomOptionCheck === 'function') {
              window.Helpers.updateCustomOptionCheck(themeInput)
            }
          }

          // Sync semi-dark section visibility
          const semiDarkL = this.container.querySelector('.template-customizer-semiDark')
          if (semiDarkL) semiDarkL.classList.add('d-none')
...
TemplateCustomizer.THEMES = [
  ...
  },
  {
    name: 'beranda',
    title: 'Beranda',
    image: '<i class="ti tabler-sparkles icon-base mb-0"></i>'
  }
]
