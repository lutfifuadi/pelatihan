  ...
  <div class="container-fluid px-4 px-lg-6 position-relative" style="z-index: 1;">
    ...
    <div class="glass-card-premium px-4 px-xl-5 py-4 mb-4">
      <div class="d-flex align-items-center gap-3">
        ...
        <div>
          <h4 class="fw-bold text-white mb-0">Edit Pelatihan</h4>
          <p class="text-body-premium mb-0 mt-1" style="font-size: 0.95rem;">
            Perbarui data program pelatihan: {{ $pelatihan->nama }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="col-12">
      <div class="glass-card-premium px-4 px-xl-5 py-5">
        <form action="{{ route('admin.pelatihan.update', $pelatihan) }}" method="POST">
          @csrf
          @method('PUT')
          ...
          <div class="mb-4">
            ...
            <input type="text" class="form-control @error('nama') is-invalid @enderror"
              id="nama" name="nama" value="{{ old('nama', $pelatihan->nama) }}" placeholder="Masukkan nama pelatihan" required>
            @error('nama')
          ...
          <div class="mb-4">
            ...
            <select class="select2 form-select @error('dinas_id') is-invalid @enderror"
              ...
              <option value=""></option>
              @foreach($dinas as $d)
                <option value="{{ $d->id }}" {{ old('dinas_id', $pelatihan->dinas_id) == $d->id ? 'selected' : '' }}>
                  {{ $d->nama_dinas }}
                </option>
              @endforeach
            </select>
          ...
          <div class="mb-4">
            ...
            <select class="select2 form-select @error('kecamatan_ids') is-invalid @enderror"
              ...
              @foreach($kecamatans as $kec)
                <option value="{{ $kec->id }}" {{ in_array($kec->id, old('kecamatan_ids', $pelatihan->kecamatans->pluck('id')->toArray())) ? 'selected' : '' }}>
                  {{ $kec->name }}
                </option>
              @endforeach
            </select>
          ...
          <div class="mb-4">
            ...
            <input type="text" class="form-control @error('batch') is-invalid @enderror"
              id="batch" name="batch" value="{{ old('batch', $pelatihan->batch) }}" placeholder="Contoh: BATCH 1" required>
            @error('batch')
              <div class="invalid-feedback mt-1">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="mb-4">
            ...
            <textarea class="form-control @error('deskripsi') is-invalid @enderror"
              id="deskripsi" name="deskripsi" rows="3" placeholder="Masukkan deskripsi pelatihan">{{ old('deskripsi', $pelatihan->deskripsi) }}</textarea>
            @error('deskripsi')
              <div class="invalid-feedback mt-1">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="row mb-4">
            <div class="col-md-6 mb-3 mb-md-0">
              ...
              <input type="date" class="form-control @error('batas_pendaftaran') is-invalid @enderror"
                id="batas_pendaftaran" name="batas_pendaftaran" value="{{ old('batas_pendaftaran', $pelatihan->batas_pendaftaran ? \Carbon\Carbon::parse($pelatihan->batas_pendaftaran)->format('Y-m-d') : '') }}">
              <small class="text-body-premium mt-1 d-block" style="font-size: 0.75rem;">
            ...
            <div class="col-md-6">
              ...
              <input type="number" class="form-control @error('kuota') is-invalid @enderror"
                id="kuota" name="kuota" value="{{ old('kuota', $pelatihan->kuota) }}" placeholder="Masukkan kuota peserta" min="0">
              @error('kuota')
                <div class="invalid-feedback mt-1">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="row mb-4">
            <div class="col-md-6 mb-3 mb-md-0">
              ...
              <input type="date" class="form-control @error('tanggal_mulai') is-invalid @enderror"
                id="tanggal_mulai" name="tanggal_mulai" value="{{ old('tanggal_mulai', $pelatihan->tanggal_mulai ? \Carbon\Carbon::parse($pelatihan->tanggal_mulai)->format('Y-m-d') : '') }}">
              @error('tanggal_mulai')
                <div class="invalid-feedback mt-1">{{ $message }}</div>
              @enderror
            </div>
            <div class="col-md-6">
              ...
              <input type="date" class="form-control @error('tanggal_selesai') is-invalid @enderror"
                id="tanggal_selesai" name="tanggal_selesai" value="{{ old('tanggal_selesai', $pelatihan->tanggal_selesai ? \Carbon\Carbon::parse($pelatihan->tanggal_selesai)->format('Y-m-d') : '') }}">
          ...
          <div class="mb-4">
            <div class="form-check d-flex align-items-center gap-2">
              ...
              <input type="checkbox" class="form-check-input @error('is_active') is-invalid @enderror"
                id="is_active" name="is_active" value="1" {{ old('is_active', $pelatihan->is_active) ? 'checked' : '' }}>
          ...
          <div class="d-flex justify-content-end gap-3 mt-5">
            ...
            <button type="submit" class="btn btn-glow-premium px-4 py-2">
              <i class="icon-base ti tabler-device-floppy me-1"></i> Perbarui
            </button>
          </div>
        </form>
      </div>
