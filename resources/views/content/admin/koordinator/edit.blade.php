  ...
  <div class="container-fluid px-4 px-lg-6 position-relative" style="z-index: 1;">
    ...
    <div class="glass-card-premium px-4 px-xl-5 py-4 mb-4">
      <div class="d-flex align-items-center gap-3">
        ...
        <div>
          <h4 class="fw-bold text-white mb-0">Edit Koordinator</h4>
          <p class="text-body-premium mb-0 mt-1" style="font-size: 0.95rem;">
            Perbarui data koordinator: {{ $koordinator->name }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="col-12">
      <div class="glass-card-premium px-4 px-xl-5 py-5">
        <form action="{{ route('admin.koordinator.update', $koordinator) }}" method="POST">
          @csrf
          @method('PUT')
          ...
          <div class="mb-4">
            ...
            <input type="text" class="form-control @error('name') is-invalid @enderror"
              id="name" name="name" value="{{ old('name', $koordinator->name) }}" required>
            @error('name')
          ...
          <div class="mb-4">
            ...
            <input type="email" class="form-control @error('email') is-invalid @enderror"
              id="email" name="email" value="{{ old('email', $koordinator->email) }}" required>
            @error('email')
              <div class="invalid-feedback mt-1">{{ $message }}</div>
            @enderror
          </div>
          ...
          <!-- Password -->
          <div class="mb-4">
            <label for="password" class="form-label">Password <small class="text-muted" style="text-transform: none;">(kosongkan jika tidak diubah)</small></label>
          ...
          <div class="mb-4">
            ...
            <select class="form-select @error('kecamatan_id') is-invalid @enderror"
              ...
              @foreach($kecamatans as $kec)
                <option value="{{ $kec->id }}" {{ old('kecamatan_id', $koordinator->kecamatan_id) == $kec->id ? 'selected' : '' }}>
                  {{ $kec->name }}
                </option>
              @endforeach
            </select>
          ...
          <div class="mb-4">
            ...
            <select class="form-select @error('kelurahan_id') is-invalid @enderror"
              id="kelurahan_id" name="kelurahan_id" required disabled>
          ...
          <div class="row mb-4">
            <div class="col-md-6 mb-3 mb-md-0">
              ...
              <input type="text" class="form-control @error('nik') is-invalid @enderror"
                id="nik" name="nik" value="{{ old('nik', $koordinator->nik) }}" placeholder="15-16 digit NIK" maxlength="16" inputmode="numeric">
              @error('nik')
                <div class="invalid-feedback mt-1">{{ $message }}</div>
              @enderror
            </div>
            <div class="col-md-6">
              ...
              <input type="text" class="form-control @error('phone') is-invalid @enderror"
                id="phone" name="phone" value="{{ old('phone', $koordinator->phone) }}">
              @error('phone')
                <div class="invalid-feedback mt-1">{{ $message }}</div>
              @enderror
            </div>
          ...
          <div class="mb-4">
            ...
            <input type="text" class="form-control @error('whatsapp') is-invalid @enderror"
              id="whatsapp" name="whatsapp" value="{{ old('whatsapp', $koordinator->whatsapp) }}">
            @error('whatsapp')
              <div class="invalid-feedback mt-1">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="d-flex gap-3 mt-5">
            ...
            <a href="{{ route('admin.koordinator.index') }}" class="btn btn-secondary-custom px-4 py-2">
              Batal
            </a>
          </div>
        </form>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (kecamatanSelect && kelurahanSelect) {
    ...
    // Load kelurahan on page load if kecamatan already selected
    var initialKecamatan = kecamatanSelect.value;
    var initialKelurahan = '{{ old('kelurahan_id', $koordinator->kelurahan_id) }}';
    if (initialKecamatan) {
      loadKelurahan(initialKecamatan, initialKelurahan);
    }
  }
});
</script>
