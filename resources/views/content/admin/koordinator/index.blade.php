    ...
    <div class="glass-card-premium px-4 px-xl-5 py-4 mb-4">
      <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div class="d-flex align-items-center gap-3">
          ...
          <div>
            <h4 class="fw-bold text-white mb-0">Kelola Koordinator</h4>
            <p class="text-body-premium mb-0 mt-1" style="font-size: 0.95rem;">
              Daftar dan kelola akun Koordinator Wilayah Kecamatan
            </p>
          </div>
        </div>
        <a href="{{ route('admin.koordinator.create') }}" class="btn btn-glow-premium px-4 py-2 d-flex align-items-center gap-2">
          <i class="icon-base ti tabler-plus"></i> Tambah Koordinator
        </a>
      </div>
    </div>
                ...
                <tr style="border-bottom: 1px solid rgba(255, 255, 255, 0.04);">
                  ...
                  <td class="text-end px-0 py-3">
                    <div class="d-inline-flex gap-2">
                      <a href="{{ route('admin.koordinator.edit', $koordinator) }}" class="btn btn-warning btn-sm d-flex align-items-center justify-content-center" style="border-radius: 5px; width: 32px; height: 32px; padding: 0;">
                        <i class="icon-base ti tabler-edit fs-5 text-dark"></i>
                      </a>
                      <form action="{{ route('admin.koordinator.destroy', $koordinator) }}" method="POST" class="d-inline"
                        onsubmit="return confirm('Yakin ingin menghapus koordinator {{ $koordinator->name }}?')">
                        @csrf
                        @method('DELETE')
    ...
    function initToggleKoordinator() {
        ...
        document.querySelectorAll('.toggle-koordinator').forEach(function(el) {
            el.addEventListener('click', function(e) {
                ...
                // Update modal content based on desiredState
                if (desiredState) {
