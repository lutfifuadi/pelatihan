...
@section('title', 'Approval Koordinator')
  ...
  <div class="container-fluid px-4 px-lg-6 position-relative" style="z-index: 1;">
    ...
    <div class="glass-card-premium px-4 px-xl-5 py-4 mb-4">
      <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        ...
        <a href="{{ route('admin.koordinator.index') }}" class="btn btn-secondary-custom px-4 py-2 d-flex align-items-center gap-2">
            ...
            <tbody>
              @forelse($koordinators as $index => $koordinator)
                <tr style="border-bottom: 1px solid rgba(255, 255, 255, 0.04);">
                  ...
                  <td class="py-3 text-body-premium">{{ $koordinator->email }}</td>
                  <td class="py-3">
                    <span class="badge-premium badge-premium-info">{{ $koordinator->kecamatan?->name ?? '-' }}</span>
                  </td>
                  <td class="py-3">
                    <a href="https://wa.me/{{ $koordinator->whatsapp }}" target="_blank" class="text-warning text-decoration-none small">
                      <i class="icon-base ti tabler-brand-whatsapp me-1"></i>{{ $koordinator->whatsapp ?? '-' }}
                    </a>
                  </td>
                  <td class="py-3 text-body-premium">{{ $koordinator->created_at->format('d M Y H:i') }}</td>
                  <td class="text-end px-0 py-3">
                    <div class="d-inline-flex gap-2">
                      <form action="{{ route('admin.koordinator.approve', $koordinator) }}" method="POST" class="d-inline">
                        @csrf
                        <button type="submit" class="btn btn-success btn-sm px-3" style="border-radius: 5px;"
                          onclick="return confirm('Aktifkan koordinator {{ $koordinator->name }}?')">
                          <i class="icon-base ti tabler-circle-check me-1"></i> Setujui
                        </button>
                      </form>
                      <form action="{{ route('admin.koordinator.reject', $koordinator) }}" method="POST" class="d-inline">
                        @csrf
                        <button type="submit" class="btn btn-danger btn-sm px-3" style="border-radius: 5px;"
                          onclick="return confirm('Hapus pendaftaran koordinator {{ $koordinator->name }}? Data akan dihapus permanen.')">
