@forelse($kecamatans as $index => $kecamatan)
  <tr style="border-bottom: 1px solid rgba(255, 255, 255, 0.04);">
    <td class="px-0 py-3 text-body-premium">{{ $kecamatans->firstItem() + $index }}</td>
    <td class="py-3 text-white fw-semibold" style="text-transform: uppercase;">{{ $kecamatan->name }}</td>
    <td class="py-3">
      @if($kecamatan->users->count() > 0)
        @foreach($kecamatan->users as $koordinator)
    ...
    <td class="text-end px-0 py-3">
      <div class="d-inline-flex gap-2">
        <a href="{{ route('admin.kecamatan.edit', $kecamatan) }}" class="btn btn-warning btn-sm d-flex align-items-center justify-content-center" style="border-radius: 5px; width: 32px; height: 32px; padding: 0;">
          <i class="icon-base ti tabler-edit fs-5 text-dark"></i>
        </a>
        <form action="{{ route('admin.kecamatan.destroy', $kecamatan) }}" method="POST" class="d-inline"
          onsubmit="return confirm('Yakin ingin menghapus kecamatan {{ $kecamatan->name }}?')">
          @csrf
          @method('DELETE')
