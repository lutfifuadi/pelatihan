...
<div class="container-fluid px-4 px-lg-6 position-relative" style="z-index: 1;">
  ...
  <nav aria-label="breadcrumb" class="mb-3">
    <ol class="breadcrumb mb-0">
      <li class="breadcrumb-item">
        <a href="{{ route('dashboard.peserta') }}" class="text-decoration-none" style="color: rgba(255,255,255,0.5); font-size: 0.85rem;">
  ...
  <div class="glass-card-premium px-4 py-3 mb-4">
    <div class="d-flex flex-wrap gap-2">
      ...
      <a href="{{ route('notifications.index', ['status' => 'all', 'channel' => request('channel')]) }}"
        class="filter-btn {{ request('status', 'all') === 'all' ? 'active' : '' }}">Semua Status</a>
      <a href="{{ route('notifications.index', ['status' => 'unread', 'channel' => request('channel')]) }}"
        class="filter-btn {{ request('status') === 'unread' ? 'active' : '' }}">Belum Dibaca</a>
      <a href="{{ route('notifications.index', ['status' => 'read', 'channel' => request('channel')]) }}"
        class="filter-btn {{ request('status') === 'read' ? 'active' : '' }}">Sudah Dibaca</a>
    </div>
  </div>
  ...
  <div class="glass-card-premium overflow-hidden p-0">
    @forelse($notifications as $notif)
      <div class="notif-item {{ $notif->read_at ? '' : 'unread' }} p-3 p-xl-4" style="border-radius: 0; border-top: none; border-left: none; border-right: none; margin-bottom: 0;">
        <div class="d-flex align-items-start gap-3">
          ...
          <div class="flex-grow-1" style="min-width: 0;">
            ...
            <p style="color: rgba(255,255,255,0.6); font-size: 0.85rem; margin: 6px 0 0 0; line-height: 1.5;">
              {{ Illuminate\Support\Str::limit($notif->body, 200) }}
            </p>
            <div class="mt-3 d-flex align-items-center gap-2 flex-wrap">
              <span class="badge-premium" style="font-size: 0.7rem;">
                <i class="icon-base ti tabler-{{ $notif->channel === 'whatsapp' ? 'brand-whatsapp' : ($notif->channel === 'email' ? 'mail' : 'bell') }} me-1"></i>
              ...
              @if($notif->data && isset($notif->data['wa_data']))
                @php
